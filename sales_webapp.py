...
def get_consolidated_vehicles():
    ...
    try:
        ...
        # Add cards for vehicles with opening balance that are not billed today
        # These are vehicles from previous month that still have pending material
        for truck_number, opening_data in opening_balance_map.items():
            if truck_number not in actual_trucks_billed_today:
                # Check if this truck has pending material (positive balance)
                opening_ppc = opening_data.get('ppc', 0)
                opening_premium = opening_data.get('premium', 0)
                opening_opc = opening_data.get('opc', 0)
                
                # Get current month's billing for this truck
                cursor.execute('''
                ...
                # Get current month's other_dealers_billing for this truck
                cursor.execute('''
                    SELECT COALESCE(SUM(ppc_quantity), 0), COALESCE(SUM(premium_quantity), 0), 
                           COALESCE(SUM(opc_quantity), 0)
                    FROM other_dealers_billing
                ...
                # Get current month's unloading for this truck
                cursor.execute('''
                ...
                if pending_ppc > 0.01 or pending_premium > 0.01 or pending_opc > 0.01:
                    # Get the most recent billing date for this truck
                    cursor.execute('''
                    ...
                    if recent_billing:
                        ...
                        dealer_name = recent_billing[2]
                        plant_depot = recent_billing[3] or 'PLANT'
                    else:
                        billing_date = opening_data.get('billing_date', 'Previous Month')
                        dealer_code = opening_data.get('dealer_code')
                        dealer_name = 'Opening Balance'
                        plant_depot = 'PLANT'
